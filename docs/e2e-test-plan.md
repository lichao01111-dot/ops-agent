# OpsAgent 端到端测试计划 (Architecture v2)
...
| **A02** | OpsAgent "查询 MySQL 地址" 走 knowledge 路由 | §5.1 | `response.route == "knowledge"`，stub 的 `query_knowledge` 被调用 1 次 |
| **A03** | OpsAgent mutation 带合法 receipt → 工具成功执行 | §4.2 #1 / §8.2 | `ToolCallEvent.status == SUCCESS`，输出不含 `error` |
...
| ID | 用例 | 架构引用 | Pass 判据 |
|---|---|---|---|
| **D01** | 两个独立 OpsAgent 实例共享 session_id，数据互不可见 | §5.5 | A 写入的记忆，B 读取为 `None` |
